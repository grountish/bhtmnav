<template>
  <nav v-if="showNav" class="">
    <ul class="flex justify-between fixed bottom-0 inset-x-0 p-6 my-2">
      ...
      <li>
        <div @click="navChapter(+1)"  @mouseover="chapterHover" @mouseleave="chapterUnhover" v-show="chapter !== 3" class="bg-pink-100 rounded-3xl border cursor-pointer border-solid border-red-200 flex flex-row mx-5 px-3 py-2 flex-1 justify-around">
          <h1 v-if="showChapNav" class="text-red-600 text-bold my-auto mx-4 text-xl">CHAPTER {{chapter + 1}}</h1>
          <button ><img src="~/assets/r-arrow.svg" class="right-arrow" alt="" /></button>
        </div>
      </li>
    </ul>
  </nav>
